...
- **직접 확인하지 못한 것 2건** — 숨은 탭에서는 `requestAnimationFrame`이 멈추고 `setTimeout`이 강하게 늦춰져서, ① 안내 자동 넘김 ② 9단계의 두 번째 커밋(`tLater` 700ms 예약)을 실제 재생으로 검증하지 못했다. 창을 앞에 두고 한 번 돌려서 확인할 것

**해결된 항목** — 원격 다중 이름표 레이아웃(`ccdade2`), rebase 반복 시 유령 겹침(`ghostLane`), 파비콘(인라인 data URI), `git fetch` 표현(`057475b`), PR 병합(`85c09ce`), 범례 아이콘 구분(버려진 커밋=점 / 아직 안 받은 역=파선)
...
|---|---|---|---|
| 살아있는 커밋 | 브랜치 색 | 실선 | 흰색 |
| 버려진 커밋 (rebase 후) | `#AEB8C4` | **점** (`0.1 5`, round cap) | 흰색, `opacity .45` |
| 아직 받지 않은 역 | `REMOTE_COLOR` | **파선** (`7 5`) | 흰색, `opacity .8` |
