...
같은 줄 오른쪽 끝에 **방금 실행한 명령**(`#curCmd`)이 붙는다. 사이드바 맨 아래 로그는 그림에서 대각선으로 가장 먼 자리라 "무엇을 쳐서 무엇이 달라졌는지"가 안 붙어 보였다. 그래프 오른쪽 빈 공간은 **쓰면 안 된다** — Merge 169px, Rebase 123px까지 좁아지고(1384px 창 기준) `.map-scroll` 안에 넣으면 가로 스크롤을 따라 흘러간다.

- 출처는 `S.log`의 **마지막 non-`"o"` 줄**이다. `"o"`는 명령이 뿌린 출력이라 건너뛰고 그 앞 명령을 잡는다(`git branch -a`). 색·`$` 접두는 로그의 `c`/`e`/`n` 규칙을 그대로 쓴다 — 자유 모드에서도 그대로 동작하고, **거절이 빨갛게 그림 옆에 뜨는 게 이 배치의 최대 이득**이다
- **`flex:1 1 0`이 핵심.** 폭 상한을 px나 %로 박으면 창 폭마다 경계가 어긋나 줄바꿈이 일어나고 범례가 2줄이 되며 노선도가 통째로 밀린다(`max-width:46%`·`300px` 둘 다 실패했다). basis 0이면 줄의 필요 폭에 0으로 잡혀 줄바꿈을 유발하지 않고, 남는 자리만 차지하다 모자라면 ellipsis로 줄어든다
- 900px 이하는 숨긴다. 한 열로 접히면 사이드바가 노선도 바로 아래로 와서 이미 가깝고, 남는 가로 자리도 없다
