...
구역 이름은 노선도 **왼쪽 세로 기둥**(44px, 모바일 37px)으로 세운다. 시작점에 이름만 찍는 가로 라벨은 "여기부터"만 말할 뿐 "어디까지"를 못 말해 경계가 안 읽혔다. 기둥은 길이 자체가 구역의 범위다. 글자는 `text-orientation: upright`로 세워 위에서 아래로 읽히고, 잉크(로컬)·`#46525F`(서버) 판에 흰 글씨를 얹어 노선도 위에서 사라지지 않는다.

**기둥은 폭을 먼저 확보하고 그래프를 양보시킨다.** 좁혀서 우겨넣으면 글자가 조용히 잘린다(세로쓰기는 넘쳐도 오류가 없다). 담기지 않는 짧은 구역에서는 `fitZone()`이 `내컴퓨터`→`로컬`로 이름을 줄인다.
