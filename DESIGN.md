...
구역 이름은 노선도 **왼쪽 세로 기둥**(38px, 모바일 33px)으로 세운다. 시작점에 이름만 찍는 가로 라벨은 "여기부터"만 말할 뿐 "어디까지"를 못 말해 경계가 안 읽혔다. 기둥은 길이 자체가 구역의 범위다. 잉크(Local)·`#46525F`(Remote) 판에 흰 글씨를 얹어 노선도 위에서 사라지지 않는다.

이름은 **`Local` / `Remote`** — 화면에서 유일하게 영문인 자리다. git 문서가 쓰는 말이고 학생이 `git branch -r`·`origin/main`에서 다시 만나므로, 기둥이 그 단어를 미리 익히는 자리가 된다. 라틴 문자는 `text-orientation`을 건드리지 않고 기본값으로 눕힌다 — 책등·간판의 방식이고, `upright`로 세우면 낱자가 다섯 칸으로 쌓여 읽기 어색하다.

**`Local`은 언제나 서고 `Remote`만 원격이 있을 때 붙는다.** 원격이 없어도 "여기는 내 컴퓨터"는 유효하다. 둘을 함께 숨기면 일곱 시나리오 중 넷에서 구역 이름이 통째로 사라진다.

**기둥은 폭을 먼저 확보하고 그래프를 양보시킨다.** 좁혀서 우겨넣으면 글자가 조용히 잘린다(세로쓰기는 넘쳐도 오류가 없다). `fitZone()`이 실제 글자 높이를 실측해 안 들어가면 폰트를 낮춘다.

## Elevation & Depth

층위는 세 겹 그림자로 만든다. 패널 기본(`0 1px 2px .07 / 0 5px 14px .07 / 0 18px 44px .10`), 노선도는 이 화면의 주인공이라 한 겹 더(`0 2px 4px .08 / 0 10px 24px .10 / 0 30px 60px .12`), 안내 말풍선에 부양 그림자(`0 18px 50px rgba(0,0,0,.32)`), 헤더 판에 `0 6px 26px rgba(20,28,38,.28)`.

**노선도 SVG 내부(역·선로)는 인쇄물 은유대로 평평하게 유지한다** — 그림자는 종이가 놓인 층위를 말하지, 인쇄된 잉크에는 걸리지 않는다.
...
**알약은 버튼의 형태이지 묶음의 형태가 아니다.** 버튼을 담는 컨테이너(헤더의 `.modes .group`)는 패널과 같은 14px를 쓴다. 999px로 두면 좁은 화면에서 라벨이 자기 줄로 올라갈 때 그 밑줄이 모서리 곡선과 어긋나 라벨이 테두리 밖으로 빠져나간 것처럼 보인다.

선 종류가 의미를 진다: **실선** = 살아있는 것, **파선(7 5)** = 서버에 있지만 아직 안 받은 것, **점선(0.1 5 round-cap)** = rebase로 버려진 것. **파선 테두리**는 HTML 쪽에서도 "잠정 상태"를 뜻한다(스테이징 상자, 흐름 띠의 아직 안 간 칸).

범례는 노선도와 같은 판을 쓰지 않는다 — `--paper` 바탕에 테두리를 둘러 **역 안내판**으로 떼어 놓고 `범례` 라벨을 붙인다. 점선 밑줄 하나로 나눴을 때는 노선도와 한 덩어리로 보여 "이건 기호 설명"이라는 게 안 읽혔다.
...
### 명령 띠 (`.cmdbar`) — 시그니처
- 방금 실행한 명령 한 줄. 높이 46px 고정(비어도 예약), mono 18.5px 700. **잉크 배경에 밝은 글씨로 진짜 터미널을 닮는다** — 노선도(밝은 인쇄물)와 터미널(어두운 화면)이 눈으로 갈리는 자리다. `$` 접두는 밝은 분홍(#FF5C9B). 거절이면 띠 전체가 #8E1F2C로 타고 글씨는 #FFE1E4(대비 7.20) — 뒷자리에서 "막혔다"가 즉시 읽힌다.
...
- **머리부(`.cap`)만 헤더와 같은 잉크 판**이고 위에 노선색 6등분 띠가 얹힌다. 흰 상자 하나뿐일 때는 화면에 떠 있는 또 하나의 패널로 보였다 — 안내판과 같은 옷을 입어야 "설명하는 목소리"로 읽힌다. 본문(`.body`)은 흰 바탕 그대로 둔다(읽는 글이 길어 반전하면 피로하다). 잉크 위에서는 `--head`가 대비 3.82로 미달이라 장 이름은 `#FF6BA3`(6.44), 단계 번호는 `#A9B4C0`(8.15)을 쓴다.
